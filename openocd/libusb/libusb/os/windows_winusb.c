...
//#define ENUM_DEBUG
		...
		for (i = 0; ; i++) {
			...
			if ((pass >= HUB_PASS) && (pass <= GEN_PASS)) {
				// ASMedia xHCI controller driver does not return the correct port number
				// for SPDRP_ADDRESS. So we try SPDRP_LOCATION_INFORMATION first.
				// If it's in the "Port_#xxxx.Hub_#xxxx" format, we parse it to get the port
				// number. Otherwise, we try SPDRP_ADDRESS.
				if (!pSetupDiGetDeviceRegistryPropertyA(dev_info, &dev_info_data, SPDRP_LOCATION_INFORMATION,
					&reg_type, (BYTE *)loc_info, sizeof loc_info, &size)) {
					usbi_warn(ctx, "could not retrieve location information for device '%s': %s",
						dev_id_path, windows_error_str(0));
					size = 0;
				}
				if (size == strlen("Port_#xxxx.Hub_#xxxx") + 1
					&& strncmp(loc_info, "Port_#", 6) == 0
					&& strncmp(loc_info + 10, ".Hub_#", 6) == 0)
					port_nr = strtol(loc_info + 6, NULL, 10);
				else if ((!pSetupDiGetDeviceRegistryPropertyA(dev_info, &dev_info_data, SPDRP_ADDRESS,
