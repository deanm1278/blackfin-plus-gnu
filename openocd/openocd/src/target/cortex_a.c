...
static int cortex_a_get_vector_base(struct target *target, uint32_t *value)
{
	struct armv7a_common *armv7a = target_to_armv7a(target);
	struct arm *arm = &armv7a->arm;
	uint32_t sctlr, vector_base;
	int retval;
	...
	/* if SCTLR.V is set, use high exception vectors */
	if (sctlr & (1 << 13))
...
static int cortex_a_step_quiet(struct target *target)
{
	...
	struct breakpoint *saved_breakpoint = NULL;
	uint8_t saved_matchmode;
	struct reg *r = arm->pc;
	...
	if (arm->is_semihosting) {
		...
		retval = cortex_a_get_vector_base(target, &vector_base);
		if (retval != ERROR_OK)
			return retval;

		if (address == vector_base + 0x8) {
			cortex_a_setup_semihosting(target, 0);
			semihosting_disabled = 1;
		}
	}

	if (cortex_a->brp_num_available < 1) {
		/* Find a hardware breakpoint which can be saved and restored easily */
		struct breakpoint *breakpoint = target->breakpoints;
		while (breakpoint) {
			if (breakpoint->type == BKPT_HARD && breakpoint->linked_BRP == 0) {
				uint32_t control = brp_list[breakpoint->set - 1].control;
				saved_matchmode = (control >> 20) & 0x7;
				saved_breakpoint = breakpoint;
				break;
			}
			breakpoint = breakpoint->next;
		}
		if (saved_breakpoint == NULL)
			return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;

		cortex_a_unset_breakpoint(target, saved_breakpoint);
	}

	/* Setup single step breakpoint */
	memset(&stepbreakpoint, 0, sizeof(stepbreakpoint));
	...
	if (saved_breakpoint != NULL)
		cortex_a_set_breakpoint(target, saved_breakpoint, saved_matchmode);
...
/*	armv4_5->exception_number = 0; */
	...
	if (DSCR_ENTRY(dscr) == DSCR_ENTRY_VECT_CATCH) {
		if (arm_semihosting(target)) {
			...
			return ERROR_OK;
		} else if (target->halt_issued) {
			target->debug_reason = DBG_REASON_DBGRQ;
			return ERROR_OK;
		} else if (!cortex_a->is_stepping) {
			/* If there is a breakpoint at the vector address, treat
			   as if hitting the breakpoint. */
			uint32_t address = buf_get_u32(arm->pc->value, 0, 32);
			if (breakpoint_find(target, address) != NULL) {
				target->debug_reason = DBG_REASON_BREAKPOINT;
				return ERROR_OK;
			}

			retval = cortex_a_step_quiet(target);
			if (retval != ERROR_OK)
				return retval;
			...
			/* If no breakpoint or watchpoint, resume. */
			retval = cortex_a_internal_restart(target);
			if (retval != ERROR_OK)
				return retval;
		}
