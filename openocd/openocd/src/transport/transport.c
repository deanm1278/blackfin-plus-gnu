...
static int jim_transport_select(Jim_Interp *interp, int argc, Jim_Obj * const *argv)
{
	...
	switch (argc) {
		case 1:	/* autoselect if necessary, then return/display current config */
			if (!session) {
				char supported_transports[256] = {'\0'};
				char one_transport[32];

				if (!allowed_transports || !allowed_transports[0]) {
					LOG_ERROR("Debug adapter does not support any transports? Check config file order.");
					return JIM_ERR;
				}

				for (unsigned i = 0; allowed_transports[i]; i++) {
					sprintf(one_transport, "\"%s\"", allowed_transports[i]);
					strcat(supported_transports, one_transport);
					if (allowed_transports[i + 1])
						strcat(supported_transports, ", ");
				}
				LOG_INFO("transports supported by the debug adapter: %s", supported_transports);

				LOG_INFO("auto-select transport \"%s\"", allowed_transports[0]);
		...
		case 2:	/* assign */
			...
			for (unsigned i = 0; allowed_transports[i]; i++) {
				if (strcmp(allowed_transports[i], argv[1]->bytes) == 0) {
					LOG_INFO("select transport \"%s\"", argv[1]->bytes);
